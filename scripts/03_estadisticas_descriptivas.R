...
# Creamos los directorios output/figures y output/tables en caso de que no existan:
dir.create("output/figures", recursive = TRUE, showWarnings = FALSE)
dir.create("output/tables", recursive = TRUE, showWarnings = FALSE)

recorridos_clean <- read_csv("data/clean/recorridos_clean.csv")

# Traducimos al español los valores de genero. Calculamos estadísticas descriptivas
# para hombres y mujeres:
recorridos_clean_estadistica <- recorridos_clean %>%
  mutate(
    genero = case_when(
...
print(recorridos_clean_estadistica)

# Generamos el gráfico de densidad y lo guardamos:
graf_densidad <- ggplot(recorridos_clean, aes(x = duracion_recorrido, color = genero)) +
...
ggsave("output/figures/grafico_densidad.png", plot = graf_densidad, width = 8, height = 5, dpi = 300, bg = "transparent")

# Generamos el boxplot y lo guardamos:
png("output/figures/boxplot.jpg", width = 1600, height = 1200, res = 150, bg = "transparent")
boxplot(recorridos_clean$duracion_recorrido)
dev.off()

# Guardamos la tabla con las estadísticas calculadas:
gt_tabla <- gt(recorridos_clean_estadistica) %>%
  cols_label(
    genero = "Género",
    obs = "Observaciones",
    media = "Media",
...
gtsave(gt_tabla, "output/tables/estadisticas_descriptivas.png")
